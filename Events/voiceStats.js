  ...
  MemberStats.findOne({guildID: sunucuAyar.guildID, userID: memberID}, (err, data) => {
    if (!data) {
      ...
      let newMember = new MemberStats({
        ...
        chatStats: chatMap,
        voiceStatsTotal: voiceMap,
        chatStatsTotal: chatMap,
        totalChatStats: 0
      });
      newMember.save();
    } else {
      let onceki = data.voiceStats.get(categoryID) || 0;
      let oncekiTotal = data.voiceStatsTotal.get(categoryID) || 0;

      data.voiceStats.set(categoryID, Number(onceki)+duraction);
      data.voiceStats.set(categoryID, Number(oncekiTotal)+duraction);
