...
module.exports.execute = async(client, message, args,ayar,emoji) => {
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    ...
    MemberStats.findOne({ guildID: message.guild.id, userID: uye.id }, (err, data) => {
        ...
        data.voiceStatsTotal.forEach(c => haftalikSesToplam += c);
        let haftalikSesListe = '';
        data.voiceStatsTotal.forEach((value, key) => haftalikSesListe += ` \`● ${message.guild.channels.cache.has(key) ? message.guild.channels.cache.get(key).name : 'Bilinmeyen'}:\` ** ${client.convertDuration(value)}**\n`);
        let haftalikChatToplam = 0;
        data.chatStatsTotal.forEach(c => haftalikChatToplam += c);
        let haftalikChatListe = '';
        data.chatStatsTotal.forEach((value, key) => haftalikChatListe += `\`● ${message.guild.channels.cache.has(key) ? message.guild.channels.cache.get(key).name : 'Bilinmeyen'}:\`** ${value} mesaj**\n`);
        ...
        embed.addField('Genel Ses',`\`● Toplam:\`  ** ${client.convertDuration(haftalikSesToplam)}** \n ${haftalikSesListe}`);
        embed.addField('Genel Chat',`\`● Toplam:\`  ** ${haftalikChatToplam} mesaj** \n ${haftalikChatListe}`);
    message.channel.send(embed)
    });
};
module.exports.configuration = {
    name: 'stat2',
    aliases: ['stats2', 'vinfo2', 'cinfo2'],
    usage: 'stat [üye]',
